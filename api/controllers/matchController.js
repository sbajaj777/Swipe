...
export const getUserProfiles = async (req, res) => {
	try {
		...
		const cityQuery = req.query.city?.trim().toLowerCase(); // Get city from query
		...
		if (cityQuery) {
			baseFilters.$and.push({
				city: { $regex: new RegExp(`^${cityQuery}$`, "i") }, // case-insensitive exact match
			});
		}

		const users = await User.find(baseFilters);
